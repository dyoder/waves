...
describe "A Waves request instance" do
  ...
  before do
    @request = Waves::Request.new(rack_env("http://example.com/index.html", 'CONTENT_TYPE' => "text/html"))
  end
...
describe "The HTTP request method" do
  ...
  it "is determined in a straightforward manner for straightforward requests" do
    @get = Waves::Request.new(rack_env("/", :method => 'GET'))
    @post = Waves::Request.new(rack_env("/", :method => 'POST'))
    @put = Waves::Request.new(rack_env("/", :method => 'PUT'))
    @delete = Waves::Request.new(rack_env("/", :method => 'DELETE'))
  ...
  it "can be set with the '_method' query param on a POST" do  
    @url_put = Waves::Request.new(rack_env("/?_method=put", :method => 'POST'))
    @url_delete = Waves::Request.new(rack_env("/?_method=delete", :method => 'POST'))
  ...
  it "can be set with the '_method' body param on a POST" do  
    @body_put = Waves::Request.new(rack_env("/", :method => 'POST', :input => '_method=put'))
    @body_delete = Waves::Request.new(rack_env("/", :method => 'POST', :input => '_method=delete'))
